...
export default function Products({
  ...
  const groupedCategoryFilters = useMemo(() => {
    ...
    const merged: Filter[] = [];
    const colorOptionsMap = new Map<number, any>();

    newFiltersData.filters.forEach((filter) => {
      const titleLower = filter.title?.toLowerCase() || "";
      const isColor = titleLower.includes("rəng") || titleLower.includes("color");

      if (isColor) {
        filter.options.forEach((opt) => {
          colorOptionsMap.set(opt.id, opt);
        });
        return;
      }

      merged.push(filter as Filter);
    });
    ...
    if (colorOptionsMap.size > 0) {
      merged.unshift({
        id: 2,
        title: "Rəng",
        options: Array.from(colorOptionsMap.values()),
      } as Filter);
    }
  ...
  const getSeoTitle = useMemo(() => {
    ...
    if (isBestseller) return `${translation?.cox_satilan || 'Best Sellers'} | Brendoo`;
    if (isLowStock) return `${translation?.stokda_az || 'Low Stock'} | Brendoo`;
    if (isTopRated) return `${translation?.ulduzlu_mehsullar || '5 Star Products'} | Brendoo`;
    if (checked) return `${translation?.Endirimli_məhsullar || 'Discounted Products'} | Brendoo`;
    return `${translation?.Məhsullar || 'Products'} | Brendoo`;
  }, [slug, category, categories, translation, isBestseller, isLowStock, isTopRated, checked, lang]);
  ...
  const FilterSection = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={isMobile ? "space-y-4" : "flex flex-col mt-2 text-black whitespace-nowrap gap-4"}>
      ...
      {/* Dinamik Filtrlər */}
      {category && category.length > 0 ? (
        newFiltersData?.filters && newFiltersData.filters.length > 0 ? (
          <DynamicFilters
            filters={groupedCategoryFilters as any[]}
            selectedOptions={options}
            onFilterChange={setoptions}
          />
        ) : newFiltersLoading ? <FilterSkeleton /> : null
      ) : filtersLoading ? <FilterSkeleton /> : groupedFilters && groupedFilters.length > 0 ? (
        <DynamicFilters
      ...
      <div className="flex gap-3 items-center self-start mt-4 font-medium text-black text-opacity-80">
        ...
        <div className="self-stretch my-auto">{translation?.Endirimli_məhsullar || 'Discounted'}</div>
      </div>
      <div className="flex gap-3 items-center self-start mt-4 font-medium text-black text-opacity-80">
        ...
        <div className="self-stretch my-auto">{translation?.cox_satilan || 'Best Sellers'}</div>
      </div>
      <div className="flex gap-3 items-center self-start mt-4 font-medium text-black text-opacity-80">
        ...
        <div className="self-stretch my-auto">{translation?.stokda_az || 'Low Stock'}</div>
      </div>
      <div className="flex gap-3 items-center self-start mt-4 font-medium text-black text-opacity-80">
        ...
        <div className="self-stretch my-auto">{translation?.ulduzlu_mehsullar || '5 Star Products'}</div>
      </div>
    </div>
  );
        ...
        <section className="flex overflow-hidden flex-col bg-black">
          <div className="flex relative flex-col pt-10 pr-20 pb-36 pl-10 w-full min-h-[324px] max-md:px-5 max-md:pb-24 max-md:max-w-full" style={{ backgroundImage: heroImage ? `url(${heroImage})` : 'linear-gradient(to right, #1a1a2e, #16213e)', backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" }}>
            <nav className="flex relative gap-2 items-center self-start text-base" aria-label="Breadcrumb">
              ...
              <span className="self-stretch my-auto text-white">{translation?.Ana_sehife || 'Home'}</span>
              </Link>
        ...
        <section className="flex flex-col w-full max-md:px-5 max-sm:px-0">
          <div className="flex lg:flex-row flex-col mt-[20px] md:mt-[60px] lg:px-[40px] px-[10px] gap-4">
            <aside className="flex flex-col w-full lg:max-w-[280px]">
              <h2 className="text-xl font-semibold text-black">{translation?.Filter || 'Filter'}</h2>
              <div className="md:hidden">
                <Suspense fallback={<SkeletonItem className="h-12 rounded-full" />}>
                  <MobileFilter translation={translation} selectedItems={{ category: category, subCategory: null, thirdCategory: null, options: options }} onClose={closeFilter}>
                    <div className="p-4">
                      <p className="text-sm text-gray-600 mb-4">{translation?.Filter_description || 'Filter products'}</p>
            ...
            <section className="flex w-full my-2 sm:my-8 flex-col rounded-none" aria-label="Məhsullar">
              <div style={{ display: slug && slug.length > 0 ? "none" : "" }} className="flex flex-wrap gap-5 items-center justify-between w-full max-md:max-w-full">
                <div className="flex gap-4 items-center flex-wrap">
                  <label htmlFor="sort-select" className="self-stretch my-auto text-sm text-black text-opacity-60">{translation?.Sırala || 'Sort'}</label>
                  <div className="flex overflow-hidden gap-10 self-stretch px-4 py-3.5 my-auto text-base font-medium text-black bg-neutral-100 rounded-[100px] lg:w-[283px] w-[200px]">
                    <select id="sort-select" onChange={(e) => setSort(e.target.value)} value={Sort} className="w-full focus:outline-none bg-[#F5F5F5]">
                      <option value="">{translation?.Sırala || 'Sort'}</option>
                      <option value="random">{translation?.Random || 'Random'}</option>
                      <option value="A-Z">A-Z</option>
                      <option value="Z-A">Z-A</option>
                      <option value="expensive-cheap">{translation?.Expensive_Cheap || 'Price: High to Low'}</option>
                      <option value="cheap-expensive">{translation?.Cheap_Expensive || 'Price: Low to High'}</option>
                      <option value="old-new">{translation?.Old_New || 'Oldest First'}</option>
                      <option value="new-old">{translation?.New_Old || 'Newest First'}</option>
                    </select>
                  </div>
                </div>
              </div>
